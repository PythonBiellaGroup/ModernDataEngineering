...
ospedale = st.sidebar.selectbox(
    "Strutture Ospedaliere", df_ang_ospedali["denom_struttura"].to_list()
)
gr_clinico = st.sidebar.selectbox(
    "Gruppo Clinico", df_ang_clinico["DESCRIZIONE_ACC_DI_DIAGNOSI"].to_list()
)
...
st.plotly_chart(
    make_plot.plt_performance_ospdeli(df_performance_ospedali), use_container_width=True
)
...
st.plotly_chart(make_plot.plt_bar_reparti(df_reparto_clinico), use_container_width=True)

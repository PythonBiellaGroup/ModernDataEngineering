...
df_performance_hospital = pd.read_sql_query(
    "SELECT * FROM dbo.EXT_PERFORMANCE_HOSPITAL", con=cxn
)
cxn.dispose()
...
df_performance_hospital.to_sql(
    "WRH_PERFORMANCE_HOSPITAL", con=cxn, if_exists="append", index=False
)
cxn.dispose()
